...
@section('content')
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-body">
                    <form role="form" action="{{route('channel.category.store')}}" method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group">
                            <label for="title">Category Name :</label>
                            <input type="text" class="form-control @error('name') is-invalid @enderror" id="title" name="name" placeholder="Category Name" value="{{old('name')}}">
                            @error('name')
                        ...
                        <div class="input-group mb-3">
                            ...
                            <input type="text" class="form-control @error('slug') is-invalid @enderror" id="slug" name="slug" placeholder="Category Slug" value="{{old('slug')}}">
                            @error('slug')
                            <span class="d-block invalid-feedback" role="alert">{{$message}}</span>
                            @enderror
                        </div>
                        <div class="form-group">
                            ...
                            <select name="homepage" id="status" class="form-control">
                                <option {{old('homepage') == 'Yes'? 'selected':''}} value="Yes">Yes</option>
                                <option {{old('homepage') == 'No'? 'selected':''}} value="No">No</option>
                            </select>
                        </div>
                        <div class="form-group">
